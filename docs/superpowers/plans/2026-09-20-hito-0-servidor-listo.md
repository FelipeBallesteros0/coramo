...
- [x] **Step 2: Reiniciar y comprobar que arrancó del SSD** — hecho. Resultado: raíz en `/dev/nvme0n1p2`, `/boot/firmware` en `/dev/nvme0n1p1`, arranque en ~50 s, lectura 434 MB/s. La microSD no queda montada; sigue intacta como rescate.

**Estado del arranque tras la migración (determinista en ambos sentidos):**

| Unidad | Etiquetas | Cómo arranca |
|---|---|---|
| SSD (en uso) | `SSDBOOT` / `writable-ssd` | `root=PARTUUID=d6b594f8-02` |
| microSD (rescate) | `system-boot` / `writable` | `root=LABEL=writable`, etiqueta que solo existe en ella |

`BOOT_ORDER=0xf416` con `PCIE_PROBE=1`: prueba NVMe, luego microSD, luego USB.

**Riesgo a vigilar:** no hay `flash-kernel` ni `/etc/default/flash-kernel`, pero `linux-firmware-raspi` gestiona `/boot/firmware/current/` con el esquema A/B (`tryboot_a_b=1`). Si una actualización de kernel regenerara `current/cmdline.txt` con `root=LABEL=writable`, apuntaría a la microSD en vez del SSD. Por eso conviene **no dejar la microSD puesta**: el fallo sería visible en vez de arrancar en silencio el sistema viejo. Tras cada actualización de kernel, comprobar `findmnt -n -o SOURCE /`.
